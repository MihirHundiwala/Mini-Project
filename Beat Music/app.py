...
def recommend():
    ...
    try:
        queue = list(usref.child('u45455').child('Queue').get().values())
        ...
        for song in queue[-5:]:
            ...
            distances, indices = model_knn.kneighbors(df.iloc[query_index, :].values.reshape(1, -1), n_neighbors=3)

            for i in range(0, len(distances.flatten())):
                if i == 0:
                    print('\n\nRecommendations for {0} ({1}):\n'.format(ref.child(
                        df.index[query_index]).get()['Title'], ref.child(df.index[query_index]).get()['Genre'],))

                else:
                    rec_list.append(df.index[indices.flatten()[i]])
                    print('{0}: {1} ({3}), with distance of {2}:'.format(i, ref.child(df.index[indices.flatten()[i]]).get()[
                          'Title'], distances.flatten()[i], ref.child(df.index[indices.flatten()[i]]).get()['Genre']))
